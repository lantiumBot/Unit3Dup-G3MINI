...
class Uploader(Tracker):
    def upload_t(self, data: dict, torrent_archive_path: str, nfo_path=None) -> requests.Response:
        ...
        if not os.path.exists(torrent_archive_path):
            custom_console.bot_error_log(f"[Upload] Fichier torrent introuvable: {torrent_archive_path}")
            raise FileNotFoundError(f"Torrent file not found: {torrent_archive_path}")
        
        # Ouvrir le fichier torrent
        torrent_file = open(torrent_archive_path, 'rb')
        files['torrent'] = ('upload.torrent', torrent_file, 'application/x-bittorrent')

        # Ajouter le fichier NFO si fourni
        nfo_file = None
        if nfo_path:
            if os.path.exists(nfo_path) and os.path.isfile(nfo_path):
                ...
                custom_console.bot_warning_log(f"[Upload] Fichier NFO introuvable: {nfo_path}")
