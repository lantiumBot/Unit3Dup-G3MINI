...
class Media:
    def __init__(self, folder: str, subfolder: str):
        ...
        # Si subfolder est vide, utiliser directement le nom du dossier
        if self.subfolder:
            self.title: str = os.path.basename(os.path.join(self.folder, self.subfolder))
        else:
            # C'est un dossier complet, utiliser le nom du dossier
            self.title: str = os.path.basename(self.folder)
    ...
    def torrent_path(self) -> str:
        if not self._torrent_path:
            if self.subfolder:
                ...
                # Si subfolder est vide, utiliser directement folder (cas d'un dossier complet)
