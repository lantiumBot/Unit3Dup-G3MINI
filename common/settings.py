...
class ConsoleOptions(BaseModel):
    ...
    WELCOME_MESSAGE: str = "G3MINI TR4CK3R"
...
class Config(BaseModel):
    ...
    uploader_tag: UploaderTag = UploaderTag()
    ...
    def set_default_tracker_config(cls, v):
        ...
        for field,value in section.items():
            if value is None:
                ...
                elif field_upper in ['GEMINI']:
                     section[field] = Validate.dict(value=section[field], field_name=field_upper)
                elif field_upper in ['MULTI_TRACKER']:
                    section[field] = Validate.validate_multi_tracker(multi_tracker_list=section[field])
                else:
                    section[field] = Validate.string(value=section[field], field_name=field_upper)
        return v
...
class JsonConfig:
    ...
    def __init__(self, default_json_path: Path):
        ...
        # New torrent attribute (only keys missing from JSON, not obsolete keys)
        expected_keys = set(TorrentClientConfig.__annotations__.keys())
        actual_keys = set(self.torrent_config.keys())
        missing_keys = expected_keys - actual_keys
        self.torrent_diff_keys = missing_keys if missing_keys else None
        ...
        # New uploader_tag attribute
        self.uploader_tag_diff_keys = self.uploader_tag_config.keys() ^ UploaderTag.__annotations__.keys()\
        if not self.uploader_tag_config.keys() == UploaderTag.__annotations__.keys() else None
    ...
    def update_uploader_tag_config(self):
        # Ajouter la section uploader_tag si absente du JSON
        if "uploader_tag" not in self.file_config_data:
            self.updated = True
            self.file_config_data["uploader_tag"] = self.uploader_tag_config
    ...
    def get_config_updated(self) -> dict:
        ...
        # Update the loaded file json section 'uploader_tag'
        self.update_uploader_tag_config()
